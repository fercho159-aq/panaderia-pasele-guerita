                ...
                <div className="animate-fade-in">
                    ...
                    <div className="grid md:grid-cols-2 gap-6 mb-10">
                        {pickupLocations.map(loc => (
                            <div
                                key={loc.id}
                                onClick={() => setSelection({ ...selection, locationId: loc.id, day: loc.days[0] })}
                                className={`p-6 rounded-2xl border-2 cursor-pointer transition-all ${selection.locationId === loc.id ? 'border-primary bg-primary/5 shadow-inner' : 'border-bg hover:border-accent'}`}
                            >
                                <h4 className="font-serif text-xl font-bold mb-2">{loc.name}</h4>
                                <p className="text-sm text-gray-500 mb-2">{loc.address}</p>
                                <p className="text-xs font-bold text-accent">{loc.days.join(' & ')} | {loc.hours}</p>
                            </div>
                        ))}
                    </div>
                    <Button
